...
// =============================================================================
// experiments — АРХИВ. В проде не запускается; хранится как песочница разных
// видов голос-реактивной анимации и как эталон измерения fps. Основной эффект
// (обёртка выреза) живёт в main.swift; выводы по производительности — в NOTES.md.
//
// Виды (все на CADisplayLink, vsync-locked 120 Гц):
//   • эквалайзер-бары (высота по голосу)
//   • пульсирующие кольца (разбегаются по голосу, запечённая текстура)
//   • эталон-спиннер + реальный счётчик fps/джиттера
//
// Запустить вручную при желании:
//   swiftc -O experiments.swift -o /tmp/exp -framework AppKit -framework QuartzCore \
//     -framework AVFoundation && OVERLAY=1 /tmp/exp
// =============================================================================
