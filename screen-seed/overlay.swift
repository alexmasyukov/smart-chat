...
final class PointsView: NSView {
    private let cubeHost = CALayer()           // хост заливок кубиков (свой цвет на ключ)
    ...
    override init(frame: NSRect) {
        ...
        lines.lineWidth = 1
        host.addSublayer(cubeHost)                 // заливки под точками
    ...
    private func colorFromHex(_ hex: String, alpha: CGFloat) -> NSColor {
        var s = hex
        if s.hasPrefix("#") { s.removeFirst() }
        guard s.count == 6, let v = UInt32(s, radix: 16) else { return NSColor(white: 0.5, alpha: alpha) }
    ...
    func update(_ points: [[Double]], _ kinds: [String], _ numbers: [Int],
        ...
        cubeHost.frame = bounds
        cubeHost.sublayers?.forEach { $0.removeFromSuperlayer() }
        for (ci, c) in cubes.enumerated() where c.count >= 4 {
