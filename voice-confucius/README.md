...
### Почему звук не улучшить

«Труба» = завал верхов (low-pass), фундаментальное ограничение модели:
выход 22050 Гц через 80-полосный мел-спектр → потолок ~11 кГц.

- **int8 ни при чём:** вокодер `bigvgan` и `s2a` в int8-версии НЕ
  квантованы — файлы байт-в-байт совпадают с fp16-портом
  (`beyoru/Confucius4-TTS-mlx`). Квант затрагивает только t2s и w2vbert.
  fp16-веса звук не спасут, а память/скорость ухудшат.
- **Шаги солвера не помогают.** Увеличение числа шагов flow-matching
  убирает шум, но яркости не добавляет (проверено, звук тот же):

  | steps | gen | RTF | звук |
  |---|---|---|---|
  | 26 (дефолт) | 3.37с | 1.20 | — |
  | 50 | 4.52с | 1.61 | без изменений |
  | 80 | 6.02с | 2.14 | без изменений |
  | 120 | 7.47с | 2.66 | без изменений |

  Параметры `steps`/`cfg` в `server.py` оставлены, но дефолт 26 —
  поднимать смысла нет.
