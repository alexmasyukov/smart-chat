...
CASES = [
    ...
    # «папку» → каталог на диске, а не стенд: интент с суффиксом _folder
    ("открой папку нетворк", "open_network_folder", {}),
    ("покажи компоненты", "open_components", {}),
    ("открой мои проекты", "open_projects", {}),
    ("зайди в адсв", "open_adsw", {}),

    # --- джира: задача по номеру и доска целиком ---
    # Различие ровно в наличии номера, и это единственный признак: «открой
    # джиру» → доска, «открой в джире задачу 175» → конкретная задача.
    ("открой в джире задачу ард 3344", "open_jira_task", {"ticket": "ард", "num": "3344"}),
    ("открой задачу 4321 в джире", "open_jira_task", {"num": "4321"}),
    ("джиру открой", "open_jira", {}),
    ("покажи нашу джиру", "open_jira", {}),
...
# Составные фразы («открой X на 2070 и Y на 3511») из обучения убраны
# намеренно: в реальном употреблении не нужны, а длина фразы из-за них росла с
# 18 слов до 42 и вдвое удорожала обучение. Архитектура их поддерживает —
# вернуть можно, подняв MULTI_COUNT в gen_slots.py и переобучив.
MULTI_CASES = []
